...
//Every manager has the same find and make methods, they just get and return their own specific type.
...
//AnchorManager contains a list of all the anchors that should point to an object. When we go to save a file, we make all these anchors.
class AnchorManager : public RoseManager, public FindMake < AnchorManager >
{
protected:
...
public:
	...
	void AddAnchor(std::string in) { anchors.push_back(in); };
	std::vector<std::string> GetAnchors() { return anchors; };
};